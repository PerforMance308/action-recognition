...
class OS_ELM(object):
    ...
    def __init__(
        self, n_input_nodes, n_hidden_nodes, n_output_nodes, C,
        activation='sigmoid', loss='mean_squared_error', name=None):
        with tf.variable_scope("os_elm", reuse=tf.AUTO_REUSE):
            if name == None:
                self.name = 'model'
            else:
                self.name = name
    ...
    def __build_init_train_graph(self):
        ...
        HTH = tf.matmul(HT, H)        
        p = tf.assign(self.__p, tf.matrix_inverse(C_I + HTH))
        pHT = tf.matmul(p, HT)
    ...
    def __build_seq_train_graph(self):
        ...
        temp = tf.matrix_inverse(C_I + HpHT)
